...
export function DiffCard({ icon: Icon, title, children }: { icon: LucideIcon; title: string; children: React.ReactNode }) {
  return (
    <div className="rounded-card border border-black/5 bg-white p-7 transition duration-200 hover:-translate-y-0.5 hover:border-paprica/30 hover:shadow-card-hover">
...
export function Highlight({ icon: Icon, title, children }: { icon: LucideIcon; title: string; children?: React.ReactNode }) {
  return (
    <div className="flex items-start gap-3 rounded-card border border-black/5 bg-white p-4">
